...
- [Usage](#usage) — the endpoint, [nonced tokens](#nonced-tokens), [chaining](#chaining), streaming
- [Command line](#command-line)
- [Configuration](#configuration)
- [How it works](#how-it-works)
- [What it deliberately does not do](#what-it-deliberately-does-not-do)
- [Development](#development)
- Full reference: [docs/](docs/) — [proxying](docs/proxying.md) · [tokens](docs/tokens.md) · [nonced](docs/nonced.md) · [deploy](docs/deploy.md)
...
- **The token travels in the URL, so use the nonced form.** A raw `px_…` in a URL is written
  into caddy's access log — it redacts `Authorization`, but this is not a header — and works
  forever once it is there. A nonced `pxc_…` proves the same token without sending it and is
  useless five minutes later. Four lines of shell, same token, same parameter:
  [Nonced tokens](#nonced-tokens).
...
| `token` | required | The credential, raw or [nonced](#nonced-tokens). Missing or unknown is a `401` |
...
A token can be sent whole, or kept back and proved with a hash that stops working in five
minutes. Same token either way — there is nothing to choose when you mint one.
...
another](#chaining) — and a URL is the part of a request that everything in front writes down.
So derive the value instead. `sha256sum` is in every base image including busybox:
...
The key is `sha256(secret)` — exactly what `tokens.json` already stores — so there is no second
kind of token, nothing new on disk, and every token you already have works this way today.

It does not make the five minutes safe, only the log afterwards, and it does not bind the value
to the URL it arrived on. [docs/nonced.md](docs/nonced.md) has the format, a TypeScript version,
why the key goes last in the hash, and what this deliberately does not do.
...
and `.`, and are the key a token is deleted by.

```
$ proxio token list
LABEL         ID        TOKEN      CREATED
some_service  64f6417e  mqMezft6…  2026-09-14T00:24:47Z
ci_runner     f62a7ec3  nmdH98co…  2026-09-14T00:24:48Z
```

Two identifiers, both unique, and neither computable from the other. **TOKEN** is the secret's
opening characters *after* the `px_` prefix — which every token has, so counting it would spend
three of the eight saying nothing — for matching a listing against a token in somebody's config
file.
**ID** comes off the hash and is what appears inside a [nonced token](#nonced-tokens), so a
request in a log can be traced to a token by someone who does not hold the secret.

There is no `token sign`: deriving a nonced value belongs to the caller, who is the only one
that should have the secret. → [docs/tokens.md](docs/tokens.md)
