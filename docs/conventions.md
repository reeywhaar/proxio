...
| **secret** | The token itself, `px_…`. Never "the token" where both could be meant |
| **id** | A token's first 8 hash characters. Public; goes on the wire |
| **hint** | A token's first 8 *secret* characters. Not public; goes in a listing |
| **nonced** | A token proved rather than sent, `pxc_…`. Never "signed" — nothing is signed |
