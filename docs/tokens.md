...
```json
{
  ...
  "tokens": [
    {
      "label": "some_service",
      "hash": "b7e1cda94dd1d240ba277eb4d831e96974b0ca6a70d17f7635a1f18ed4fa0a72",
...
`hint` is the secret's first eight characters **after the `px_` prefix**, kept so a listing can
be matched against a token sitting in somebody's config file. The prefix is on every token, so
keeping it would spend three of the eight characters saying something already known. A token
minted before the field existed has none and cannot grow one — it comes off the secret, which is
gone; one minted before the prefix was dropped still carries it, and the listing trims it.

### Two identifiers, and why neither replaces the other

| | read off | where it appears | unique |
| --- | --- | --- | --- |
| **hint** | the **secret**, after `px_` | `token list`, as `mqMezft6…` | yes |
| **id** | the **hash** — its first 8 characters | `token list`, and inside every [nonced token](nonced.md) | yes |

They answer different questions and neither can be computed from the other, which is the point.
The hint matches what a caller holds; the id matches what arrives on the wire. The id is safe to
put in a URL and the hint is not, because the hint is the beginning of the secret.

Both are unique, and `Create` re-mints on a clash of *either* — they are independent, so a fresh
secret that clears one can still collide on the other. Eight characters is 32 bits for the id
and 48 for the hint, which is far more than a handful of tokens needs; the loop is there
so a collision is impossible rather than merely unlikely, and two rows showing the same
identifier would make a listing lie.
...
`hint` did not need one: an added optional field means an older file reads as a token without a
hint, which is exactly what it is.
...
A token can be sent whole, or kept back and proved with a hash that is useless five minutes
later. Same token either way, nothing to decide at mint time.
...
It matters here more than it would anywhere using a header: proxio's credential travels in a
URL so that [one proxio can stand in front of another](proxying.md#chaining), and a URL is the
part of a request that everything in front writes into a log.

The whole of it — the format, the shell and TypeScript recipes, why the key is the stored hash,
why plain SHA-256, and what it deliberately does not do — is in [nonced.md](nonced.md).
