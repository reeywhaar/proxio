...
| [tokens.md](tokens.md) | The file, the two identifiers, the hashing, how a running server sees `docker exec` |
| [nonced.md](nonced.md) | Proving a token without sending it, so a URL in a log stops being a credential |
...
proxio is small — about 1,800 lines of Go under `internal/` and rather more than that in test beside
