...
| `token` | yes | The credential, raw (`px_…`) or [nonced](nonced.md) (`pxc_…`). Missing or unknown is a `401` |
...
`token` and nothing else, in one of two forms told apart by their prefix, so neither can be
presented as the other:

| | | |
| --- | --- | --- |
| `px_…` | the secret itself | works forever, and is a credential wherever it lands |
| `pxc_…` | a [nonced token](nonced.md) | proves the secret without sending it; useless after five minutes |

Both are accepted and there is nothing to choose at mint time — the second is derived from the
first. **Use the nonced form in anything that is not a one-off**, because this credential
travels in a URL and a URL is what everything in front of proxio writes into a log. See
[Where the token ends up](#where-the-token-ends-up).

Missing, empty, unrecognised, or a nonced value outside its window is a `401` — **the same
status and the same body for all of them**, because the difference tells whoever is guessing
which half they got right. A nonced refusal is explained in proxio's *log*, where clock skew is
worth naming and no stranger can read it.
...
A credential in a URL is in every place that URL goes: the access log of every proxy in front
of proxio, browser history, shell history, and anywhere somebody pastes it. That is the cost of
putting it in a URL, and putting it in a URL is what makes
[chaining](#chaining) possible at all.

**[Nonced tokens](nonced.md) are the answer to it.** `pxc_…` proves the secret without sending
it and stops working five minutes later, so a URL that has been written down is a record of a
request rather than a way to make one. Nothing else changes: same token, same parameter, derived
by the caller in four lines of shell.

```
?token=px_XCKKOs_wu0DuJ4EUbYDOhwV7hOGbGyrVfeT_BWsOhFE   ← in the log forever, and still works
?token=pxc_1789343452.b7e1cda9.e95a47206414e7d1c60a44…  ← in the log forever, and does not
```

It does not make the five minutes safe, only the log afterwards: a value captured from a log
being written *now* works until its window closes, and nothing binds it to the URL it arrived
on. [What it does not do](nonced.md#what-it-does-not-do) is explicit about this.

**proxio's own logs are clean either way.** It never logs its own request URI — which is where
its own token is — and of a *target* URL it logs only the scheme, host and path:
...
The `?{redacted}` marker is there so a reader can tell a URL that had a query from one that did
not, rather than going looking for a request that matches nothing they sent.

**What is in front of proxio is not.** caddy logs `request.uri` in full — it redacts
`Authorization`, but this is not a header. Nonce the token and it no longer matters; if you
also want the parameter out of caddy's log, its `filter` encoder can redact a query parameter
from `request>uri`.
