...
// nonced is what a caller puts in the URL when it does not want its credential written into
// the access log of everything in front of proxio.
func (h *harness) nonced() string { return tokens.Sign(h.secret, time.Now()) }

func TestANoncedTokenAuthenticates(t *testing.T) {
	target := echoTarget(t)
	h := front(t)

	resp := h.get(t, url.Values{ParamURL: {target.URL}, ParamToken: {h.nonced()}})
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("status = %d, want 200", resp.StatusCode)
	}
	e := decode(t, resp)
	// The credential that travelled is not the secret, and none of it reaches the target.
	if strings.Contains(e.URI, h.secret) {
		t.Error("the secret reached the target")
	}
}

// Both forms work. A raw token is fine where it never leaves the caller; a nonced one is what
// survives being logged.
func TestBothKindsOfTokenAreAccepted(t *testing.T) {
	target := echoTarget(t)
	h := front(t)

	for name, token := range map[string]string{"raw": h.secret, "nonced": h.nonced()} {
		t.Run(name, func(t *testing.T) {
			resp := h.get(t, url.Values{ParamURL: {target.URL}, ParamToken: {token}})
			if resp.StatusCode != http.StatusOK {
				t.Fatalf("status = %d, want 200", resp.StatusCode)
			}
		})
...
// The whole point: yesterday's access log is not a working credential.
func TestAnExpiredNoncedTokenIsRefused(t *testing.T) {
	target := echoTarget(t)
	h := front(t)

	for name, age := range map[string]time.Duration{
		"just outside the window": tokens.Window + time.Minute,
		"an hour old":             time.Hour,
		"a day old":               24 * time.Hour,
	} {
		t.Run(name, func(t *testing.T) {
			stale := tokens.Sign(h.secret, time.Now().Add(-age))
			resp := h.get(t, url.Values{ParamURL: {target.URL}, ParamToken: {stale}})
			if resp.StatusCode != http.StatusUnauthorized {
				t.Fatalf("status = %d, want 401 for a value signed %s ago", resp.StatusCode, age)
			}
			if resp.Header.Get(HeaderError) != kindAuth {
				t.Errorf("%s = %q, want %q", HeaderError, resp.Header.Get(HeaderError), kindAuth)
			}
...
// The reply says nothing, so it is no oracle. The log says why, because a caller whose clock
// is out fails every request and "missing or names no token" points at the wrong thing.
func TestAClockSkewRefusalIsExplainedInTheLogOnly(t *testing.T) {
	target := echoTarget(t)
	h := front(t)

	stale := tokens.Sign(h.secret, time.Now().Add(-time.Hour))
	resp := h.get(t, url.Values{ParamURL: {target.URL}, ParamToken: {stale}})

	// Nothing in the response distinguishes this from an unknown token.
	unknown := h.get(t, url.Values{ParamURL: {target.URL}, ParamToken: {"px_wrong"}})
	if body(t, resp) != body(t, unknown) {
		t.Error("an expired nonce answers differently from an unknown token, which is an oracle")
	}

	logs := h.waitForLog(t)
	if !strings.Contains(logs, "clock") {
		t.Errorf("the log does not point at the clock:\n%s", logs)
	}
	if strings.Contains(logs, h.secret) {
		t.Error("the secret is in the log")
	}
}

// A nonced value names its token by id, so an operator can tell which token a request used
// without the secret ever existing on the wire.
func TestTheWireValueNamesTheTokenByID(t *testing.T) {
	h := front(t)
	...
	if !strings.Contains(h.nonced(), list[0].ID()) {
		t.Errorf("the wire value does not carry the token's id %q", list[0].ID())
	}
	// And it carries nothing that identifies the secret itself.
	if strings.Contains(h.nonced(), list[0].Hint) {
		t.Error("the wire value carries the secret's opening characters")
	}
}

// Chaining and nonced tokens are the two things the query parameter exists for, and they have
// to work together: the inner hop's credential is inside the outer hop's URL, which is exactly
// the string that ends up in a reverse proxy's log.
func TestAChainCanBeNoncedAtEveryHop(t *testing.T) {
	target := echoTarget(t)
	inner := front(t)
	outer := front(t)

	innerURL := inner.link(url.Values{ParamURL: {target.URL}, ParamToken: {inner.nonced()}})
	resp := outer.get(t, url.Values{ParamURL: {innerURL}, ParamToken: {outer.nonced()}})
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("status = %d, want 200", resp.StatusCode)
	}
	...
	for name, secret := range map[string]string{"outer": outer.secret, "inner": inner.secret} {
		if strings.Contains(e.URI, secret) {
			t.Errorf("the %s hop's secret reached the target", name)
		}
	}
	// Neither raw secret ever existed in a URL, so neither can be in either proxio's log.
	for name, h := range map[string]*harness{"outer": outer, "inner": inner} {
		logs := h.waitForLog(t)
		if strings.Contains(logs, outer.secret) || strings.Contains(logs, inner.secret) {
			t.Errorf("a raw secret is in the %s proxio's log:\n%s", name, logs)
		}
	}
}
